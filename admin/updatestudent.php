	...
	<form style="width:40%; margin: auto; margin-top:40px" enctype="multipart/form-data" method="post">
		<table>
			<tr>
				...
				<td> <label for="exampleInputPassword1">Rollno</label></td>
				<td><input type="text" class="form-control" id="exampleInputPassword1" placeholder="Enter Rollno" name="rollno" required></td>
	...
	$Query=mysqli_query($conn,"select * from student where rollno='$rollno' && name LIKE '%$name%'");
	...
	?>
		<table style="width:80%; margin:auto; border:1;">
			...
			<a href="updateform.php?id=<?php echo $fetchdata['id']?>" style='float:right; margin-right:80px; text-decoration:none; margin-top:-2px;'>Edit</a>
		<?php  }
} 
?>
</body>
